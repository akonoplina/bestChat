...
export default class UserComponent extends Component {
  onLogoutButtonPress(e) {
    e.preventDefault();
    this.props.userExit(this.props.userName, this.props.userAvatar, this.props.userAboutMe,
        this.props.userAge);
  }
  showMore() {
  ...
  render() {
    ...
    return (<Form horizontal className={'userWrapper ' + (!showUser ? 'none' : '')}>
      <FormGroup className="userAvatar">
        <Col sm={4}>
          {(userAvatar.length !== 0) ? <img role="presentation" src={require('../pics/' + userAvatar + '.png')} /> : 'no avatar yet'}
        </Col>
      </FormGroup>
      <FormGroup className={'showMoreButton ' + (showMore ? 'none' : '')} onClick={this.showMore.bind(this)}>
        <Col sm={4}>
      ...
      <FormGroup className={'showLessButton ' + (!showMore ? 'none' : '')} onClick={this.showLess.bind(this)}>
        <Col sm={4}>
          <Button bsStyle="primary">
              Show less
          </Button>
